(function(){
  ...
  async function run(){
    try{
      ...
      cards.innerHTML=`
      <div class="card"><div class="adv-head"><div><div class="adv-kicker">RAW + MODEL AYRIMI</div><h2 style="margin-top:5px">Gelişmiş Opsiyon İstihbaratı</h2></div><span class="pill-raw">RAW: OI / VOLUME</span></div><div class="adv-grid"><div class="stat-grid">${typeof stat==='function'?stat('DVOL / Current IV',iv.current_dvol==null?'—':Number(iv.current_dvol).toFixed(2),'historical vol','Historical volatility/DVOL serisinin son değeri.')+stat('IV Rank',pct(iv.iv_rank_pct),'min-max konumu','(Current − min)/(max − min).')+stat('IV Percentile',pct(iv.iv_percentile_pct),'tarihsel yüzdelik','Geçmiş gözlemlerin yüzde kaçının mevcut DVOL altında/eşit olduğunu gösterir.')+stat('Zero Gamma',a.zero_gamma==null?'—':'$'+Math.round(a.zero_gamma).toLocaleString(),'proxy','Public OI üzerinden calls + / puts − varsayımıyla hesaplanan zero-gamma proxy.'):' '}</div><div class="adv-note">IV Rank ile IV Percentile ayrı hesaplanır. GEX ve Charm dealer yönünü public OI'dan göremediğimiz için kesin dealer exposure değil; açıkça proxy olarak gösterilir.</div></div></div>
      <div class="card"><div class="adv-head"><h2>Options Volume · Strike · 24s</h2><span class="pill-raw">RAW VOLUME</span></div>${volChart(a.options_volume_by_strike)}<div class="adv-note">Call ve put barları son 24 saat kontrat hacminden doğrudan gruplanır.</div></div>
      <div class="card"><div class="adv-head"><h2>Put / Call & Max Pain · Vade Bazlı</h2><span class="pill-raw">OI + VOLUME</span></div>${expiryTable(a.expiry_profile)}<div class="adv-note">OI PCR = Put OI / Call OI · Volume PCR = 24s Put Volume / Call Volume. Max Pain her vadenin kendi OI zincirinden hesaplanır.</div></div>
